...
export class UserController {
  ...
  restoreMany(@Body() payload: DeleteUsersDto): Promise<UserInfoResponse[]> {
    return this.userService.restore(payload.ids)
  }

  @Patch('status')
  @UsePipes(new ZodValidationPipe(updateUsersStatusSchema))
  updateStatus(@Body() payload: UpdateUsersStatusDto): Promise<UserInfoResponse[]> {
  ...
  @Delete()
  @UsePipes(new ZodValidationPipe(deleteUsersSchema))
  removeMany(@Req() request: FastifyRequest, @Body() payload: DeleteUsersDto): Promise<UserInfoResponse[]> {
