...
import { columns, PostsCard, PostsDialogs, PostsProvider } from '@/features/system/posts'

import { DataTable } from '../../a2ui/components/data-table'
import { ViewSwitcher } from '../../generative-ui/components/view-switcher'

import type { JobProfile, PageMeta } from '@zen/shared'
...
function PostsCardScroll({ data }: { data: JobProfile[] }) {
  ...
  const scrollByDirection = (direction: -1 | 1) => {
    ...
    const track = viewport.querySelector<HTMLElement>('[data-slot="post-card-track"]')
    const firstCard = track?.querySelector<HTMLElement>('[data-slot="post-card-item"]')
    if (!firstCard) return
  ...
  return (
    <div className="@container relative w-full">
      ...
      >
        <div data-slot="post-card-track" className="-ml-2 flex">
          {data.map((post) => (
            <div
              key={post.id}
              data-slot="post-card-item"
...
export function PostsList({ title, description, data, loading = false }: PostsListViewProps) {
  const views = useMemo(
    () => [
      {
        key: 'card',
        ...
        render:
          data.length === 0 && !loading ? (
            ...
            <PostsCardScroll key={data.map((post) => post.id).join(':')} data={data} />
          )
      },
      {
        key: 'table',
