...
import type { RoleListItem, RoleStatus } from '@zen/shared'

type RolesBulkActionsProps = {
  selectedItems: RoleListItem[]
      ...
      { ids: targets.map((role) => role.id), status },
      {
        onSuccess: (result) => {
          if (result.failedCount === 0) {
            toast.add({ title: `已${actionText} ${result.successCount} 个角色`, type: 'success' })
          } else {
            toast.add({
              title: `已${actionText} ${result.successCount} 个角色，${result.failedCount} 个失败`,
              type: 'warning'
            })
          }
          onClearSelection()
