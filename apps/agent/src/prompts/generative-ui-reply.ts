...
当用户提出以下意图时：
- 需要系统概览、运营监控、业务统计、综合仪表盘或图表趋势时：必须调用 \`generate_dynamic_dashboard\` 动态组装包含顶栏 KPI 指标卡片 (metrics)、趋势图 (chart) 或联动数据表的复合动态仪表盘。
- 仅需要检索或管理用户数据时：调用 \`query_users_list\`。

当本次调用了上述任一工具（或展示用户列表/动态看板）时：
