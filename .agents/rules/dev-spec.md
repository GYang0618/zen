...
- **类型规范**:
  - 关于undefined和null的定义：前端优先使用 undefined，仅在“明确表达空值”时使用 null，后端以 null 为主语义，undefined 只作为“未传参/未处理”的内部状态，不进入接口协议
...
### 9.1 新增 / 编辑表单的承载方式
- **按规模选择**: 创建或编辑功能不得无条件使用 `Dialog` 或 `Sheet`（抽屉）。应以用户实际可见的字段数量为主要依据，并结合字段复杂度、帮助说明、分组数量和操作链路综合判断。
  - **轻量表单（通常不超过 6 个可见字段）**：优先使用 `Dialog`，适合无需频繁滚动、可快速完成的任务。
  - **中大型表单（通常 7 个及以上可见字段）**：优先使用 `Sheet`，为分组、说明和滚动内容提供更稳定的纵向空间。
  - **超长或多步骤表单**：若 `Sheet` 仍会造成明显的认知或操作负担，应使用独立页面或步骤表单，不得强行塞入弹层。
- **动态字段计数**: 条件字段、数组字段和展开后才出现的字段应计入复杂度判断，不得只统计初始状态字段。
- **响应式适配**: 同一操作在窄屏下可切换为全宽 `Sheet`，但必须保持字段顺序、校验反馈和提交行为一致。

### 9.2 CRUD 交互与模块基准
- **参考实现**: 表格的查询、筛选、分页、排序、批量操作、行操作，以及新增、编辑、删除和详情交互，优先参考 `apps/web/src/features/system/users` 模块。
- **复用模式**: 优先沿用 users 模块的数据表格拆分、Provider / Dialog 管理、Query / Mutation、加载态、空态、错误态和删除确认模式。
- **适配而非复制**: 参考其职责边界和交互一致性；业务规则不同时应按当前领域建模，不得直接复制与当前业务无关的字段、状态或请求逻辑。

### 9.3 Shadcn 组件优先
- **禁止重复造轮子**: 当 `packages/ui` 子包或 shadcn registry 已提供对应组件时，必须优先通过 `@zen/ui` 的公共导出直接使用或组合这些组件，不得使用原生元素加自定义样式重新实现其外观、状态和交互。
- **先检索再实现**: 编写 Button、Input、Select、Dialog、Sheet、Popover、Dropdown Menu、Tabs、Table、Form、Toast 等常见 UI 前，必须先检查项目已有 shadcn 组件及其现有用法。
- **允许的自定义范围**: 业务页面的组合布局可使用 Tailwind CSS；仅允许通过组件公开的 `variant`、`size`、`className` 等接口做必要适配，不得复制组件内部样式或破坏其可访问性。

### 9.4 Tailwind CSS 主题与深色模式
- **主题变量优先**: 颜色必须优先使用语义化主题类，如 `bg-background`、`bg-muted`、`text-foreground`、`text-muted-foreground`、`border-border`、`bg-destructive`，不得无理由使用 `bg-red-*`、`text-slate-*` 等固定色阶。
- **固定颜色例外**: 品牌色、数据可视化、状态标识或第三方规范等确需固定颜色时，必须说明其语义，并同时提供对应的 `dark:` 样式或验证该颜色在深色主题下仍满足可读性和对比度要求。
- **禁止硬编码视觉值**: 项目主题已有 token 时，不得再使用任意值语法或内联样式重复定义相同颜色、圆角、阴影和间距。

### 9.5 响应式布局
- **容器驱动**: 业务区块优先根据其容器宽度调整布局；适合独立嵌入的组件优先使用 Tailwind Container Queries，页面级骨架可使用 viewport breakpoint。
- **渐进布局**: 列数、宽度、高度、间距和信息密度应随可用空间变化，例如从单列渐进为双列或多列，不得只针对桌面固定尺寸。
- **避免脆弱尺寸**: 除业务明确要求外，避免固定宽高；优先使用 `min-*`、`max-*`、`w-full`、`h-auto`、`grid`、`flex-wrap` 和溢出策略。
- **内容可用性**: 窄屏下不得出现关键操作不可见、表单字段被截断或页面横向溢出；表格应根据业务选择滚动、隐藏次要列或切换卡片布局。

### 9.6 Tailwind 预设值优先
- **使用设计尺度**: 优先使用 Tailwind 提供的命名或预设尺度，如 `text-xs`、`text-sm`、`p-4`、`gap-2`、`rounded-md`，不得在存在等价预设值时使用任意值。
- **任意值限制**: 仅当 Tailwind 无等价 token，且该值确实来自设计稿、计算关系或第三方嵌入约束时，才允许使用 `text-[10px]`、`w-[37rem]` 等任意值，并确保响应式和主题适配。
- **禁止伪造工具类**: 不得书写 Tailwind 未定义的类名（如默认配置下的 `text-2.5`）；应先选择最接近的预设值，再评估是否需要合法的任意值。
