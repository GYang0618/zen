    ...
    <div className="flex flex-1 flex-col gap-4 p-4 md:p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">能力插件 jobs：同步假执行器，创建后立即完成</p>
        </div>
      ...
      <ul className="space-y-2">
        {(listQuery.data ?? []).map((job) => (
          <li key={job.id} className="flex items-start justify-between gap-3 rounded-lg border px-3 py-2">
